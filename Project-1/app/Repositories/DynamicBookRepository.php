...
class DynamicBookRepository implements DynamicBookRepositoryInterface
{
    public function hotel_book($request){
        ...
        {
            if($request['count_room_C1']==0 && $request['count_room_C2']==0 && $request['count_room_C4']==0 &&$request['count_room_C6']==0){
                return 8;
            }
                $data=[
    ...
    public function store_User($request){
        ...
        if($request['place_ids'] !=null)
        {
            foreach($request['place_ids'] as $place) {
                $trip_price+=Place::where('id', $place)->first()->place_price*$request['number_of_people'];
            }
        }
        if($trip_price>Bank::where('email',auth()->user()['email'])->first()->money){
            return 55;
    ...
    public function showDynamicTrip($id)
    {
        ...
        $dynamic_trip=[
            ...
            'rooms'=>$book->rooms->map(function($room) {
                return [
                    'id' => $room->id,
                    'capacity'=>$room->capacity,
                    'price' => $room->pivot->current_price,
                ];
            })
    ...
    {//where('type','dynamic')->
        ...
        //data for dynamic trip
        $dynamicBookFinished=Booking::where('user_id',auth()->id())->where('start_date','<',$date)->where('type','dynamic')
                             ->AvailableRooms()
                             ->get();
        $dynamicBookFuture=Booking::where('user_id',auth()->id())->where('start_date','>=',$date)->where('type','dynamic')
                             ->AvailableRooms()
                             ->get();
        //data for hotel trip
        $hotelBookFinished=Booking::where('user_id',auth()->id())->where('start_date','<',$date)->where('type','hotel')
                             ->AvailableRooms()
                             ->get();
        $hotelBookFuture=Booking::where('user_id',auth()->id())->where('start_date','>=',$date)->where('type','hotel')
                             ->AvailableRooms()
                             ->get();
        //data for plane trip
        $planeBookFinished=Booking::where('user_id',auth()->id())->where('start_date','<',$date)->where('type','plane')
                             ->AvailableRooms()
                             ->get();
        $planeBookFuture=Booking::where('user_id',auth()->id())->where('start_date','>=',$date)->where('type','plane')
                             ->AvailableRooms()
                             ->get();
        return [
            'finished_trip'=>[
                'dynamic_trips'=>$dynamicBookFinished,
                'hotel_trip'=>$hotelBookFinished,
                'plane_trip'=>$planeBookFinished,
            ],
            'future_trip'=>[
                'dynamic_trips'=>$dynamicBookFuture,
                'hotel_trip'=>$hotelBookFuture,
                'plane_trip'=>$planeBookFuture,
            ]
        ];
    ...
    public function get_all_plane_trip()
    {
        ...
        */
        ...
        $planeBookFuture=Booking::where('user_id',auth()->id())->where('start_date','>=',$date)->where('type','plane')
            ...
            return [
                'finished_trip'=>$planeBookFinished,
                'future_trip'=>$planeBookFuture
        ];
    }
    public function get_all_hotel_trip()
    {
        ...
        */
        ...
        $hotelBookFuture=Booking::where('user_id',auth()->id())->where('start_date','>=',$date)->where('type','hotel')
            ...
            return [
                'finished_trip'=>$hotelBookFinished,
                'future_trip'=>$hotelBookFuture
        ];
    }
    public function get_all_dynamic_trip()
    {
        ...
        $dynamicBookFuture=Booking::where('user_id',auth()->id())->where('start_date','>=',$date)->where('type','dynamic')
            ...
            return [
                'finished_trip'=>$dynamicBookFinished,
                'future_trip'=>$dynamicBookFuture
        ];
    }
    ...
    public function checkHotel($request,$id){
        try{
            if($request['hotel_id'] != null)
            {
                ...
                $rooms_1 = Room::available($request['start_date'], $request['end_date'])
                            ->where('hotel_id', $request['hotel_id'])
                            ->where('capacity', 1)
                            ->count();
                $price_rooms_1=Room::available($request['start_date'], $request['end_date'])
                ->where('hotel_id', $request['hotel_id'])
    ...
    public function bookHotel($request,$id){
        try{
            ...
            if($request['hotel_id'] != null)
            {   // rooms
                            if($request['count_room_C1']!=null)
                            {
                            $rooms = Room::available($request['start_date'], $request['end_date'])
                            ->where('hotel_id', $request['hotel_id'])
                            ->where('capacity', 1)
                            ->get();
                            for ($i = 0; $i < $request['count_room_C1']; $i++) {
               ...
               //$date = $date->addDays(1);
                ...
                if($request['end_date'] != $booking['end_date'])
                {
                    foreach($bookRoomCount as $count){
                        if(Room::where('id',$count->room_id)->first()->capacity ==1){
                            $count_1++;
                        }
                ...
                if($request['end_date'] != $booking['end_date']){
                    ...
                    $data=[
                        ...
                        'end_date'=>$request['end_date'],
                        'hotel_id'=>$hotel_id,
                        'count_room_C1'=>$request['count_room_C1']+$count_1,
                    ...
                    if($request['hotel_id'] != null)
                    {
                        if($request['count_room_C1']==null && $request['count_room_C2']==null && $request['count_room_C4']==null &&$request['count_room_C6']==null){
                            return 8;
                        }
                        $data=[
                            'start_date'=>$booking['start_date'],
    ...
    public function get_all_plane_book(){
        $plane=BookPlane::wherehas('planetrip.plane',function($query) {
            $query->where('airport_id',Airport::where('user_id',auth()->id())->first()->id);
        })->with('planetrip.booking.user',
        'planetrip.airport_source','planetrip.airport_destination',
        'planetrip.country_source','planetrip.country_destination','planetrip.plane'
        // => function($query) {
        //     $query->where('airport_id',Airport::where('user_id',auth()->id())->first()->id);
        // }
        )
